...
AI_RESULTS_FILE = "3_results.json"
VISUALS_DIR = "visuals"
...
# Import video engine module
try:
    spec = importlib.util.spec_from_file_location("VideoEngine", "10_video_engine.py")
    VideoEngine = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(VideoEngine)
except Exception as e:
    print(f"❌ Error: Could not load 10_video_engine.py: {e}")
    sys.exit(1)
...
def generate_voiceover_script(fight_brain_output):
    ...
    try:
        ...
        value_bets = fight_brain_output.get('value_bets', {})
        ...
        # Check for specific advantages in value_bets or other fields
        reason = value_bets.get('reason', '')
        ...
        if value_bets.get('pick'):
            script += f"Betting value detected: {value_bets['pick']}. "
